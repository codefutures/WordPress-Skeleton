	...
	switch( $_POST['Submit-Login-Security-Radio'] ) {
		...
		if ( ! empty($delete_users) ) {
			...
			foreach ( $delete_users as $delete_user ) {
				...
				echo $bps_topDiv;
				$textDelete = '<font color="green">'.$row->username.__(' has been deleted from the Login Security Database Table.', 'bulletproof-security').'</font><br><div class="bps-message-button" style="width:90px;"><a href="'.admin_url( 'admin.php?page=bulletproof-security/admin/login/login.php' ).'">'.esc_attr__('Refresh Status', 'bulletproof-security').'</a></div>';
				echo $textDelete;
				echo $bps_bottomDiv;	
				}
			}
		...
		if ( ! empty($unlock_users) ) {
			...
			foreach ( $unlock_users as $unlock_user ) {
				...
				echo $bps_topDiv;
				$textUnlock = '<font color="green">'.$row->username.__(' has been Unlocked.', 'bulletproof-security').'</font><br><div class="bps-message-button" style="width:90px;"><a href="'.admin_url( 'admin.php?page=bulletproof-security/admin/login/login.php' ).'">'.esc_attr__('Refresh Status', 'bulletproof-security').'</a></div>';
				echo $textUnlock;
				echo $bps_bottomDiv;	
				}			
			}
		...
		if ( ! empty($lock_users) ) {
			...
			foreach ( $lock_users as $lock_user ) {
				...
				echo $bps_topDiv;
				$textLock = '<font color="green">'.$row->username.__(' has been Locked.', 'bulletproof-security').'</font><br><div class="bps-message-button" style="width:90px;"><a href="'.admin_url( 'admin.php?page=bulletproof-security/admin/login/login.php' ).'">'.esc_attr__('Refresh Status', 'bulletproof-security').'</a></div>';
				echo $textLock;
				echo $bps_bottomDiv;
				}			
			}
